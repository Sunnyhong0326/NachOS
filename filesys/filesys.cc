...
FileSystem::FileSystem(bool format)
{
    ...
    if (format)
    {
        ...
        DEBUG('d', "Directory File Size"<< DirectoryFileSize);
...
int FileSystem::Create(char *name, int initialSize)
{
    ...
    char* finalName = strtok(name, "/");
    //char finalName[10];
    while(finalName != NULL) {
        int sector = directory->FindDir(finalName);
        if (sector == -1){
           break;
        }
        currDirFile = new OpenFile(sector);
        directory->FetchFrom(currDirFile);
        finalName = strtok(NULL, "/");
    }
    if (directory->FindFile(finalName) != -1){
        DEBUG(dbgFile, "File "<<  finalName << " is already in directory");
...
//----------------------------------------------------------------------
bool FileSystem::Remove(char *name, bool isRecur)
{
    Directory *directory;
    PersistentBitmap *freeMap;
    FileHeader *fileHdr;
    int sector = DirectorySector;
    int dirParentSector = DirectorySector;
    char cwd[270] = {'\0'};
    char buf[270] = {'\0'};
    char finalName[15] = {'\0'};
    OpenFile* currDirFile = directoryFile;
    ...
    directory->FetchFrom(directoryFile);
    char* fileName = strtok(cwd, "/");

    while(fileName != NULL) {
        dirParentSector = sector;
        sector = directory->FindDir(fileName);
        if (sector == -1){
            break;
        }
    ...
    // if final name is dir sector = current directory sector 
    // if final name is file sector = -1
    if(fileName!=NULL) { // is a file
        sector = directory->FindFile(fileName);
        strcpy(finalName, fileName);
    }
    else if (sector != -1 and isRecur) // is a directory
    {
        DirectoryEntry* table = directory->GetTable();
        for (int i = 0 ; i < directory->GetTableSize() ; i++){
            if(table[i].inUse) {
                sprintf(buf, "%s/%s", cwd, table[i].name);
                Remove(buf, true);
            }
        }
        currDirFile = new OpenFile(dirParentSector);
        directory->FetchFrom(currDirFile);
    }
    if (sector == -1) {
       delete directory;
...
void FileSystem::List(char* name, bool isRecur)
{
    Directory *directory = new Directory(NumDirEntries);
    OpenFile* currDirFile = directoryFile;
    directory->FetchFrom(directoryFile);
    ...
    while(dirName != NULL) {
        int sector = directory->FindDir(dirName);
        if (sector == -1){
            break;
        }
        currDirFile = new OpenFile(sector);
...
bool FileSystem::CreateDirectory(char* name)
{
    ...
    OpenFile *currDirFile = directoryFile;
    FileHeader *dirHdr = new FileHeader;
    currDirectory->FetchFrom(directoryFile);
    freeMap->FetchFrom(freeMapFile);
    bool success = true;
    char* dirName = strtok(name, "/");

    while(dirName != NULL) {
        int sector = currDirectory->FindDir(dirName);
        if (sector == -1){
            break;
        }
        currDirFile = new OpenFile(sector);
        currDirectory->FetchFrom(currDirFile);
        dirName = strtok(NULL, "/");
    }
    int newSector = freeMap->FindAndSet();
    if (newSector == -1) {
        DEBUG('f', "No free sector");
        success = FALSE;
    }
    if(!currDirectory->Add(dirName, newSector, true))success = FALSE;
    ...
    currDirectory->WriteBack(currDirFile);
    freeMap->WriteBack(freeMapFile);
