...
int OpenFile::ReadAt(char *into, int numBytes, int position)
{
    ...
    memset(buf, 0, sizeof(char) * numSectors * SectorSize);
    for (i = firstSector; i <= lastSector; i++)
        kernel->synchDisk->ReadSector(hdr->ByteToSector(i), &buf[(i - firstSector) * SectorSize]);
...
int OpenFile::WriteAt(char *from, int numBytes, int position)
{
    ...
    DEBUG('d', "WriteAt numSectors:" << numSectors << "\n");
    buf = new char[numSectors * SectorSize];
    ...
    DEBUG('d', "WriteAt firstSector:" << firstSector << "\n");
    DEBUG('d', "WriteAt lastSector:" << lastSector << "\n");
    // write modified sectors back
    for (i = firstSector; i <= lastSector; i++) {
        kernel->synchDisk->WriteSector(hdr->ByteToSector(i), &buf[(i - firstSector) * SectorSize]);
