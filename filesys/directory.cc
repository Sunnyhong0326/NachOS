...
{
    DEBUG('f', "Into Directory WriteBack");
    (void)file->WriteAt((char *)table, tableSize * sizeof(DirectoryEntry), 0);
    DEBUG('f', "Directory WriteBack Sucess");
}
...
int Directory::FindFile(char* name)
{
    int i = FindIndex(name);
...
{
    Directory *subDirectory = new Directory(64);
    OpenFile *currDir;
    for (int i = 0; i < tableSize; i++){
        if (table[i].inUse){
            ...
            printf("[%c] %s\n", type, table[i].name);
            if (table[i].isDir){
                currDir = new OpenFile(table[i].sector);
                subDirectory->FetchFrom(currDir);
