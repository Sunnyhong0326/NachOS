...
#define MaxFileSize1 (NumDirect * SectorSize) // 4KB (3.75KB)
#define MaxFileSize2 (NumDirect * NumDirect * SectorSize) // 128 KB (112.5KB)
#define MaxFileSize3 (NumDirect * NumDirect * NumDirect * SectorSize) // 4MB (3.29MB)
#define MaxFileSize4 (NumDirect * NumDirect * NumDirect * NumDirect * SectorSize) // 128 MB (98.87MB)
...
public:
	...
	~FileHeader();
	int MultiLevelAllocate(PersistentBitmap *freeMap, int totalSectors, int level, FileHeader *hdr);
	...
	void WriteBack(int sectorNumber); // Write modifications to file header
  	...
  	int FindSector(int offset, int level, FileHeader *hdr);
