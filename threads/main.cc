...
static void Copy(char *from, char *to)
{
    ...
    // Open UNIX file
    if ((fd = OpenForReadWrite(fileName, FALSE)) < 0)
    {
    ...
    strcpy(keepOriginalTemp, to);
    // Create a Nachos file of the same length
    DEBUG('f', "Copying file " << fileName << " of size " << fileLength << " to file " << to);
    if (kernel->fileSystem->Create(to, fileLength)==0)
    { // Create Nachos file
...
{
    // MP4 Assignment
    DEBUG('f', "Create Directory");
    bool success = kernel->fileSystem->CreateDirectory(name);
}
...
int main(int argc, char **argv)
{
    ...
    {
        if(!kernel->fileSystem->Remove(removeFileName, recursiveRemoveFlag)){
            printf("Falied to Remove file: %s", removeFileName);
        }
    }
